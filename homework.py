...
class Training:
    ...
    def get_spent_calories(self) -> float:
        ...
        raise NotImplementedError(
            'Переопределите метод get_spent_calories в дочернем классе'
        )
...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        return (self.MULTIPLIER_CALORIES * self.get_mean_speed()
                - self.LINE_SHIFT) * self.weight / self.M_IN_KM * (
            self.duration * self.MINUTES_IN_THE_HOUR)
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self):
        return (self.COEFF_CALORIES_SW_1 * self.weight
                + (self.get_mean_speed() ** 2 // self.height)
                * self.COEFF_CALORIES_SW_2 * self.weight
                ) * (self.duration * self.MINUTES_IN_THE_HOUR)
...
class Swimming(Training):
    ...
    def get_spent_calories(self) -> float:
        return (self.get_mean_speed() + self.COEFF_CALORIES_SWIM_1
